...
export default function NewInvoicePage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t.btnNewInvoice}</h1>

      <Card>
        <CardHeader><CardTitle>{t.invCustomer}</CardTitle></CardHeader>
        <CardContent className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2"><Label>{t.fldInvoiceNumber}</Label>
            <Input value={form.invoiceNumber} onChange={(e) => setForm({ ...form, invoiceNumber: e.target.value })} placeholder={t.phAutoNumber} /></div>
          <div className="space-y-2"><Label>{t.fldCustomerName}</Label>
            <Input value={form.buyerName} onChange={(e) => setForm({ ...form, buyerName: e.target.value })} /></div>
          <div className="space-y-2"><Label>{t.fldOrgNumberShort}</Label>
            <Input value={form.buyerOrgNumber} onChange={(e) => setForm({ ...form, buyerOrgNumber: e.target.value })} placeholder="556677-8899" /></div>
          <div className="space-y-2"><Label>{t.fldVatNumberShort}</Label>
          ...
          <div className="space-y-2"><Label>{t.fldIssueDate}</Label>
            <Input type="date" value={form.issueDate} onChange={(e) => setForm({ ...form, issueDate: e.target.value })} /></div>
          <div className="space-y-2"><Label>{t.fldDueDate}</Label>
        ...
        <CardContent className="space-y-3">
          {lines.map((l, i) => (
            <div key={i} className="grid grid-cols-12 gap-2">
              <Input className="col-span-5" placeholder={t.phDescription} value={l.description} onChange={(e) => setLine(i, { description: e.target.value })} />
              <Input className="col-span-2" type="number" min="0" step="0.5" value={l.quantity} onChange={(e) => setLine(i, { quantity: Number(e.target.value) })} placeholder={t.phQuantity} />
              <Input className="col-span-2" type="number" min="0" step="0.01" value={l.unitPrice} onChange={(e) => setLine(i, { unitPrice: Number(e.target.value) })} placeholder={t.phUnitPrice} />
              ...
              <button className="col-span-1 text-red-600" onClick={() => setLines((ls) => ls.length > 1 ? ls.filter((_, idx) => idx !== i) : ls)} aria-label={t.ariaRemoveRow}>×</button>
            </div>
          ))}
      ...
      <Card>
        <CardContent className="space-y-3 p-5">
          <label className="flex items-start gap-2 text-sm">
            ...
            <span>{t.invReversePre} &quot;{REVERSE_CHARGE_TEXT}&quot;. {t.invReversePost}</span>
          </label>
          <div className="text-sm">
            <div className="flex justify-between"><span>{t.invSubtotal}</span><span>{kr(totals.subtotal)} kr</span></div>
            <div className="flex justify-between"><span>{t.invColVat}</span><span>{reverseCharge ? t.invVatReverse : `${kr(totals.vatTotal)} kr`}</span></div>
            <div className="flex justify-between font-semibold"><span>{t.invToPay}</span><span>{kr(totals.total)} kr</span></div>
          </div>
